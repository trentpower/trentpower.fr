...
_PUBLIC_FILES = ("js/theme.js", "sw-register.js", "js/reveal.js",
                 "en-au/index.html", "fr/index.html", "styles.css")
...
class PureChecks(unittest.TestCase):
    ...
    def test_app_js_data_nav_state_banned(self):
        self.assertTrue(any("data-nav-state" in e for e in vnr.check_app_js('el.setAttribute("data-nav-state","x")')))
    ...
    def test_index_nav_toggle_back_fails(self):
        ...
        self.assertTrue(any("nav-toggle markup is back" in e for e in vnr.check_index_html("x", html)))

    def test_index_missing_section_fails(self):
        html = _good_index().replace('<section id="contact"><h2>contact</h2></section>\n', "")
        self.assertTrue(any("contact" in e and "not found" in e for e in vnr.check_index_html("x", html)))

    def test_styles_css_nav_toggle_selector_fails(self):
        self.assertTrue(any(".nav-toggle" in e for e in vnr.check_styles_css(".nav-toggle { display:none; }\n")))
    ...
    def test_styles_src_nav_links_selector_fails(self):
        self.assertTrue(any(".nav-links" in e for e in vnr.check_styles_src(".nav-links { gap:1rem; }\n")))
...
class Evaluate(unittest.TestCase):
    def setUp(self):
        ...
        for rel in _PUBLIC_FILES:
            _write(self.root, f"public/{rel}", (REPO_ROOT / "public" / rel).read_text(encoding="utf-8"))
        _write(self.root, "styles/styles.src.css",
    ...
    def test_injected_data_nav_state_breaks(self):
        ...
        p.write_text('el.setAttribute("data-nav-state","open");\n' + p.read_text(encoding="utf-8"), encoding="utf-8")
