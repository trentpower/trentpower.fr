...
With --review DIR the validator also renders every page to PNG, writes a
contact sheet and a machine-readable layout-report.json into DIR, so a human
can eyeball the exact pages the findings point at. Review artefacts are
generated output and stay out of git.
...
def render_review(path: pathlib.Path, out_dir: pathlib.Path, findings) -> None:
    """Render every page to PNG plus a contact sheet and a JSON report."""
    ...
    for pno, page in enumerate(doc, start=1):
        ...
        png = out_dir / f"page-{pno:02d}.png"
        pix.save(png)
        page_pngs.append(png)
    n_pages = len(page_pngs)
    doc.close()
    # contact sheet: 4 columns of half-size thumbnails, row-major.
    try:
        from PIL import Image
    except ImportError:
        print("validate-layout: Pillow not available — contact sheet skipped")
    else:
        if page_pngs:
            cols = 4
            first = Image.open(page_pngs[0])
            thumb_w, thumb_h = first.width // 2, first.height // 2
            rows = (n_pages + cols - 1) // cols
            sheet = Image.new("RGB", (cols * thumb_w, rows * thumb_h), "white")
            for i, png in enumerate(page_pngs):
                r, c = divmod(i, cols)
                tile = Image.open(png).convert("RGB").resize((thumb_w, thumb_h))
    ...
    report = {
        ...
        "pages": n_pages,
        "findings": [
            {"severity": s, "page": p, "kind": k, "message": m} for (s, p, k, m) in findings
        ],
    }
    (out_dir / "layout-report.json").write_text(json.dumps(report, indent=2) + "\n")
    print(f"validate-layout: review artefacts in {out_dir}/ ({n_pages} pages + contact sheet)")
...
def main(argv=None):
    ...
    ap.add_argument("--review", metavar="DIR", help="render page PNGs + contact sheet + report")
    ...
    if args.review:
        render_review(path, pathlib.Path(args.review), findings)
