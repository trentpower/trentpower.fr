...
(function () {
  ...
  function feedback(trigger) {
    ...
    if (trigger.getAttribute('data-copy-mode') === 'cite') {
      ...
      // micro-int #10 · also signal the citation source text so it
      // can flicker italic→roman→italic. micro-interactions.js listens
      // for this event and toggles .is-acknowledging on .tp-citation.
      // the in-place "Cited · Edition …" label above remains the
      // primary confirmation (per the no-floating-feedback rule);
      // the source flash is a second quiet beat.
      try {
