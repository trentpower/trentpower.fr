...
(function () {
  ...
  function openOverlay(overlay, trigger, opts) {
    ...
    // to apply the blur to .site / #main behind the scrim. the
    // matching `body.modal-open { overflow: hidden }` declaration
    // (added in phase 96) carries the scroll-lock so we don't need
    // an inline-style write here.
  ...
  function closeOverlay(opts) {
    ...
    // matching close to the open path; the scroll-lock comes off
    // when `body.modal-open` is removed (css `overflow: hidden`).
  ...
  // `inert` attribute yet, so background layers (the gate background,
  // closed modal scrims) ship without it and we graft it on at boot.
  // openOverlay/closeOverlay still toggle inert dynamically via
  // setInert(); this just sets the initial state so the page is
  // accessible-correct from first paint.
  if (supportsInert) {
