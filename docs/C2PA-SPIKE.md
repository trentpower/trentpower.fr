...
- A **sidecar / external manifest** for the PDF was not proven in this spike and
  is the only remaining PDF path. If pursued, the sidecar `.c2pa` file becomes a
  new public artefact and must enter `integrity.json`, the public-exposure
  manifest, and the source mirrors.

Acceptance criterion "at least `README.pdf` is signed" from the plan is **not
achievable embedded**. It must be relaxed to "or a clear spike explains why not"
