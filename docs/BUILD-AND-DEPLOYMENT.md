...
3. **Deployment** publishes the artefact. A push to `main` triggers the
   GitHub Actions SFTP workflow; stage 11 of the build performs that
   push behind a typed `DEPLOY` confirmation. Deployment never rebuilds
   and never signs.
...
| 11    | DEPLOY                 | Typed `DEPLOY` confirmation, then `git push origin main` — the push triggers the GitHub Actions SFTP deploy; `gh run watch` follows the run when available.                                                                       | The deploying push                                        |
