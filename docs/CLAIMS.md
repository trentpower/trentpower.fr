...
| `OSV` | Dependencies are scanned for known vulnerabilities with osv-scanner on every pull request. | medium | `docs/SECURITY-PIPELINE.md`<br>`docs/ASSURANCE-CASE.md` | `control_osv` | pr-gate | enforced | no | maintainer | 2026-06-18 |
...
| `REUSE` | Licensing follows the REUSE specification, linted on every pull request. | medium | `README.md`<br>`docs/SECURITY-PIPELINE.md` | `control_reuse` | pr-gate | enforced | no | maintainer | 2026-06-18 |
...
- **OSV / REUSE required-status promotion.** OSV (sca) and REUSE (reuse) run green in CI but are recorded here as release_blocking false until the protect-main / protect-preprod rulesets add them to required status checks (a separate governance step). When that lands, their enforced_at gains `ruleset` and release_blocking flips true.
